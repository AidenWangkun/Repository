...
`.`匹配单个任意字符，如果要匹配`.`本身，使用`\.`进行转义

# 3-使用

正则表达式百度百科

https://baike.baidu.com/item/%E6%AD%A3%E5%88%99%E8%A1%A8%E8%BE%BE%E5%BC%8F/1700215?fr=ge_ala#7

则表达式由一些普通字符和一些[元字符](https://baike.baidu.com/item/元字符/6062776?fromModule=lemma_inlink)（metacharacters）组成。普通字符包括大小写的字母和数字，而元字符则具有特殊的含义，我们下面会给予解释。
